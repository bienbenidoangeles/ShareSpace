...
extension ChatTableView {
    @objc private func showActionSheet(){
        ...
        let blockButton = UIAlertAction(title: blockButtonTitle, style: .destructive) { (action) in
            let alertVC = UIAlertController(title: "Would you like to \(self.blockButtonTitle) this user?", message: nil, preferredStyle: .alert)
        ...
        let actions = [reportButton,detailsButton,blockButton,cancelButton]
    ...
    public func updateUI() {
        ...
        DatabaseService.shared.isUserBlocked(selfId: userId, blockedId: otherValidUser) { (result) in
            switch result{
            case .failure(let error):
                self.controller?.showAlert(title: "Error", message: error.localizedDescription)
            case .success(let bool):
